...
class CurrentShow extends DeepShow {
  int showPixelSize = 80;
  int showPixelCount = showW*showH/showPixelSize;
  int showPixelCols = showW/showPixelSize;
  int showPixelRows = showH/showPixelSize;

  int drumCount = showPixelCount;
  int bassCount = showPixelCount;
  int mainCount = showPixelCount;

  int minBarLength = 2;

  int[] bars = new int[showPixelCount];
  ...
  void setupSpecific(PApplet applet) {
    ...
    int j = 0;
    for (int i = 0; i < showPixelCount; ++i) {
      int barLength = int(random(minBarLength, showPixelRows*3));
      i += barLength;
      bars[j] = barLength;
      j++;
    }
...
class BarRect extends HRect {
  int alpha;
  BarRect(int size, int xposition, int yposition) {
    super(size);
